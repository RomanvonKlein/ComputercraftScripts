...
local blocks = {
    ...
    ["projectx:xycronium_ore"] = {["drops"] = {["projectx:xycronium_crystal"] = 4}},--TODO:Damage-values
    ["projectred-exploration:ore"] = {["drops"] = {["projectred-core:resource_item"] = 2, ["projectred-exploration:ore"]=1}},--TODO:Damage-values
    ["aroma1997sdimension:miningore"] = {["drops"] = {["minecraft:clay_ball"] = 1, ["minecraft:slime_ball"] = 1}},--TODO:Damage-values
}
...
local valuableOres = {
    ...
    "thermalfoundation:ore",--TODO:Damage-values
    "thermalfoundation:ore_fluid",
    "projectred-exploration:ore",--TODO:Damage-values
    "techreborn:ore"--TODO:Damage-values
}
...
function stepForward()
    if (isFuelled() and turtle.forward()) then
        if (direction == "x+") then
        ...
        if (breakStuff and turtle.dig()) then
            if (stepForward()) then
                if (direction == "x+") then
                    distanceFromHome.x = distanceFromHome.x + 1
                elseif (direction == "x-") then
...
function stepUp()
    if (isFuelled() and turtle.up()) then
        distanceFromHome.z = distanceFromHome.z + 1
        return true
    else
        if (breakStuff and turtle.digUp()) then
            if (stepUp()) then
                distanceFromHome.z = distanceFromHome.z + 1
                return true
            else
...
function stepDown()
    if (isFuelled() and turtle.down()) then
        distanceFromHome.z = distanceFromHome.z - 1
        return true
    else
        if (breakStuff and turtle.digDown()) then
            if (stepDown()) then
                distanceFromHome.z = distanceFromHome.z - 1
            else
                return false
...
for k = 1, sessionLength, increment do
    ...
    if ((k % 2 == 0) and selectItem("minecraft:torch")) then
        turnLeft()
        placeBlockFront("minecraft:torch")
        turnRight()
    end
    turnRight()
end
